...
struct File_Tokenizer {
  ...
  */
  ...
  [[nodiscard]] Token make_token (Token::Kind kind, String value = {}) const {
...
static Token read_symbolic_token (File_Tokenizer &tokenizer) {
  ...
  if (string_view == "struct") return tokenizer.make_token(Token::Struct);
  if (string_view == "use")    return tokenizer.make_token(Token::Use);
...
int main (int argc, char **argv) {
  ...
  auto module = LLVMModuleCreateWithName("eko");

  auto func_type = LLVMFunctionType(LLVMVoidType(), nullptr, 0, false);
  auto func = LLVMAddFunction(module, "main", func_type);

  auto block = LLVMAppendBasicBlock(func, "block");

  auto builder = LLVMCreateBuilder();
  LLVMPositionBuilderAtEnd(builder, block);

  auto tmp = LLVMBuildAdd(builder, LLVMGetParam(func, 0), LLVMGetParam(func, 1), "tmp");
